...
public class RobustDiffRep {
	...
	// each of these keys sever as a base for a group name
	...
	public RobustDiffRep(String specName, SpecScope scope, String outputLoc, Map<String, Set<String>> boundaries,
			Map<String,String> jsonStrs, Map<String, List<String>> jsonLists) {
		this.specName = specName;
		this.specScope = scope;
		this.outputLocation = outputLoc;
		this.boundariesByGroup = boundaries;
    	...
    	this.jsonStrs.put(diffRepStateEmptyKey, boundaries.isEmpty() ? TRUE : FALSE);

    	final String groupNamesKey = keyForSpecScope(specScope, GROUP_NAMES, GROUP_NAMES1, GROUP_NAMES2);
    	this.jsonLists.put(groupNamesKey, new ArrayList<>(boundaries.keySet()));
	}
	
	public void writeBoundariesByGroup() {
    	final String fileKeyBase = keyForSpecScope(specScope, DIFF_REP_FILE, DIFF_REP_FILE1, DIFF_REP_FILE2);
		for (String groupName : this.boundariesByGroup.keySet()) {
	...
	public void writeBoundaryFOLSeparatorFileByGroup(final TLC tlcTypeOK) {
		final ExtKripke stateSpaceKripke = tlcTypeOK.getKripke();
    	final Set<String> stateSpace = Utils.stateSetToStringSet(stateSpaceKripke.reach());

		for (String groupName : this.boundariesByGroup.keySet()) {
			final Set<String> group = this.boundariesByGroup.get(groupName);
			writeBoundaryFOLSeparatorFileForGroup(groupName, group, stateSpace);
		}
	}
	
	private void writeBoundaryFOLSeparatorFileForGroup(final String groupName, final Set<String> posExamples, final Set<String> stateSpace) {
    	final Set<String> negExamples = ExtKripke.setMinus(stateSpace, posExamples);
    	...
    	if (constValueVars.size() > 0) {
    		final String keyBase = keyForSpecScope(specScope, CONST_VALUE_CONSTRAINT, CONST_VALUE_CONSTRAINT1, CONST_VALUE_CONSTRAINT2);
    		final String constValueConstraintKey = keyBase + UNDERSCORE + groupName;
    	...
    	if (nonConstValueVars.size() > 0) {
    		final String separatorFileKeyBase = keyForSpecScope(specScope, SEPARATOR_FILE, SEPARATOR1_FILE, SEPARATOR2_FILE);
    		final String sortsMapFileKeyBase = keyForSpecScope(specScope, SORTS_MAP_FILE, SORTS_MAP1_FILE, SORTS_MAP2_FILE);
        	...
        	final String separatorFile = buildAndWriteSeparatorFOL(groupName, posExamples, negExamples, varTypes, nonConstValueVars, nonConstValueTypes, valueToConstantMap);
        	final String sortsMapFile = writeSortsMap(nonConstValueTypes, this.specName, this.outputLocation);
        	this.jsonStrs.put(separatorFileKey, separatorFile);
        	this.jsonStrs.put(sortsMapFileKey, sortsMapFile);
    	}
	}
    
    private String buildAndWriteSeparatorFOL(final String groupName, final Set<String> posExamples, final Set<String> negExamples, final Map<String, StateVarType> varTypes,
    	...
    	final String path = this.outputLocation + separatorFile;
